...
Pendant une installation ou une mise à jour, la page de l’instance bascule sur l’onglet Terminal et affiche les sorties stdout/stderr réelles de l’installateur en temps réel. Jusqu’à 10 000 lignes de l’installation courante sont relues depuis le disque après une navigation, un rechargement ou une reconnexion SSE ; les invites sans saut de ligne sont également affichées et les lignes visibles peuvent être copiées en une action. Lorsqu’une action humaine est nécessaire, le job passe à `waiting_for_user` et conserve une action explicite dans la page Jobs et sur l’instance. Pour Hytale, les phrases et erreurs du downloader restent intactes dans le terminal, complétées par des diagnostics de phase et de sortie DMX, tandis que seuls le code OAuth et les paramètres éphémères de l’URL sont masqués dans les journaux publics et restent disponibles dans la carte sécurisée. Le code expiré est remplacé automatiquement dans cette carte dès que le downloader en émet un nouveau. Les installateurs n’acceptent jamais de saisie shell arbitraire.
