...
sudo ln -sfn /usr/lib/dmx-server-manager/releases/1.0.13-<sha256> /usr/lib/dmx-server-manager/current
...
```bash
cosign verify-blob \
  --bundle dmx-server-manager-v1.0.13-x86_64-unknown-linux-gnu.tar.gz.sha256.sigstore.json \
  --certificate-identity https://github.com/thefrcrazy/DmxServerManager/.github/workflows/release.yml@refs/tags/v1.0.13 \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  dmx-server-manager-v1.0.13-x86_64-unknown-linux-gnu.tar.gz.sha256
cosign verify-blob \
