...
sudo ln -sfn /usr/lib/dmx-server-manager/releases/1.0.2-<sha256> /usr/lib/dmx-server-manager/current
...
Relancez la commande proposée par le panneau, qui appelle `install.ps1 -Version 1.0.2 -ExpectedArchiveSha256 <checksum-signé>`. Lorsque ce paramètre est présent, aucun fichier `.sha256` distant n’est consulté. Chaque archive reste sous `%ProgramFiles%\DmxServerManager\releases\<version>-<sha256>` et `current` est une jonction vers la release active. L’installateur restaure la jonction, la configuration SCM, l’environnement et l’état antérieur si le service ou son healthcheck HTTP échoue.

## Docker

```bash
cd install/linux
export DMX_VERSION='1.0.2'
...
```bash
cosign verify-blob \
  --bundle dmx-server-manager-v1.0.2-x86_64-unknown-linux-gnu.tar.gz.sha256.sigstore.json \
  --certificate-identity https://github.com/thefrcrazy/DmxServerManager/.github/workflows/release.yml@refs/tags/v1.0.2 \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  dmx-server-manager-v1.0.2-x86_64-unknown-linux-gnu.tar.gz.sha256
cosign verify-blob \
