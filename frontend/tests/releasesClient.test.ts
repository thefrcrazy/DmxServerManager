...
const status = {
    ...
    latest: {
        ...
        target: {
            ...
            pull_command: `DMX_VERSION='1.0.1' DMX_IMAGE='ghcr.io/thefrcrazy/dmx-server-manager@sha256:${"a".repeat(64)}' sudo --preserve-env=DMX_VERSION,DMX_IMAGE ./bootstrap-docker.sh direct && docker compose pull`,
            apply_command: `DMX_VERSION='1.0.1' DMX_IMAGE='ghcr.io/thefrcrazy/dmx-server-manager@sha256:${"a".repeat(64)}' sudo --preserve-env=DMX_VERSION,DMX_IMAGE ./bootstrap-docker.sh direct && docker compose up -d`,
        },
    },
    error_code: null,
} as const;
