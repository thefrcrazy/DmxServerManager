...
async fn security_headers(
    ...
    headers.insert(
        ...
        HeaderValue::from_static(
            "default-src 'self'; base-uri 'none'; frame-ancestors 'none'; object-src 'none'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://shared.akamai.steamstatic.com https://static-cdn.jtvnw.net; connect-src 'self'",
        ),
    );
    headers.insert(
